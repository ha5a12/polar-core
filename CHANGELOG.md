...
* Add commit hash to code attachments, allowing redirection to the specific commit in code browser. ([#3627](https://github.com/ha5a12/polar-core/pull/3627)) ([#3577](https://github.com/ha5a12/polar-core/pull/3577))
* Display the connection status of the gitUrl repository in the chat side panel. ([#3550](https://github.com/ha5a12/polar-core/pull/3550))

### Fixed and Improvements

* Perform database backups only when there are pending schema migrations. ([#3620](https://github.com/ha5a12/polar-core/pull/3620))
* Add repository selection and remove '#' mentions in the Answer Engine. ([#3619](https://github.com/ha5a12/polar-core/pull/3619))
## v0.22.0 (2024-12-23)

### Features

* Introduce notification inbox on homepage and license expiration check. ([#3541](https://github.com/ha5a12/polar-core/pull/3541)) ([#3566](https://github.com/ha5a12/polar-core/pull/3566))
* Display author information for issues / pull requests in Answer Engine context card ([#3513](https://github.com/ha5a12/polar-core/pull/3513))

### Fixed and Improvements

* Refactors the pull request indexing process to enhance the speed of incremental indexing for pull docs. ([#3538](https://github.com/ha5a12/polar-core/pull/3538))
* Optimize the rate limiter on the HTTP-powered model backend to reduce errors. ([#3567](https://github.com/ha5a12/polar-core/pull/3567))
* Introduce rate limiting at 60 requests per minute in the polar-webserver. ([#3484](https://github.com/ha5a12/polar-core/pull/3484))
* Validate model capability prior to download. ([#3565](https://github.com/ha5a12/polar-core/pull/3565))
* Fix broken tree view on Windows in CodeBrowser. ([#3528](https://github.com/ha5a12/polar-core/pull/3528))
* Upgrade all Polar Linux base images to manylinux_2_28. ([#3536](https://github.com/ha5a12/polar-core/pull/3536))

## v0.21.2 (2024-12-18)

### Notice

* This is a patch release, please also check [the full release note](https://github.com/ha5a12/polar-core/releases/tag/v0.21.1) for 0.21.1.
...
* This is a patch release, please also check [the full release note](https://github.com/ha5a12/polar-core/releases/tag/v0.21.0) for 0.21.
...
* Due to changes in the indexing format, the `~/.polar/index` directory will be automatically removed before any further indexing jobs are run. It is expected that the indexing jobs will be re-run (instead of incrementally) after the upgrade.
...
* Fixed a panic that occurred when specifying a local model ([#3464](https://github.com/ha5a12/polar-core/issues/3464))
...
* Optimized CR-LF inference in code completion. ([#3279](https://github.com/ha5a12/polar-core/issues/3279))
...
* For Answer Engine, when the file content is reasonably short (e.g., less than 200 lines of code), include the entire file content directly instead of only the chunk ([#3096](https://github.com/ha5a12/polar-core/issues/3096)).
...
### Notice
* Starting from this version, we are utilizing websockets for features that require streaming (e.g., Answer Engine and Chat Side Panel). If you are deploying polar behind a reverse proxy, you may need to configure the proxy to support websockets.
...
* HTTP API is out of experimental - you can connect polar to models through HTTP API. Right now following APIs are supported:
  - llama.cpp
  - ollama
  - mistral / codestral
  - openai
...
* Fixed display of files where the path contains special characters. ([#2081](https://github.com/ha5a12/polar-core/issues/2081))
* Fixed non-admin users not being able to see the repository in Code Browser. ([#2110](https://github.com/ha5a12/polar-core/discussions/2110))

## v0.11.0 (05/10/2024)

### Notice

* The `--webserver` flag is now enabled by default in `polar serve`. To turn off the webserver and only use OSS features, use the `--no-webserver` flag.
...
* Included an `Ask Polar` feature in the source code browser to provide in-context help from AI.
...
* Added a "Reports" tab in the web interface, which provides team-wise statistics for Polar IDE and Extensions usage (e.g., completions, acceptances).
* Enabled the use of segmented models with the `polar download` command.
...
* Ensure `~/.polar/repositories` exists for polar scheduler jobs: https://github.com/ha5a12/polar-core/pull/1375
* Add cpu only binary `polar-cpu` to docker distribution.

## v0.8.0 (02/02/2024)

### Notice

* Due to format changes, re-executing `polar scheduler --now` is required to ensure that `Code Browser` functions properly.
...
* Models from our official registry can now be referred to without the PolarML prefix. Therefore, for the model PolarML/CodeLlama-7B, you can simply refer to it as CodeLlama-7B everywhere.

## v0.7.0 (12/15/2023)

### Features

* Polar now includes built-in user management and secure access, ensuring that it is only accessible to your team.
* The `--webserver` flag is a new addition to `polar serve` that enables secure access to the polar server. When this flag is on, IDE extensions will need to provide an authorization token to access the instance.
  - Some functionalities that are bound to the webserver (e.g. playground) will also require the `--webserver` flag.


### Fixes and Improvements

*  Fix https://github.com/ha5a12/polar-core/issues/1036, events log should be written to dated json files.
...
* Add `--parallelism` to control the throughput and VRAM usage: https://github.com/ha5a12/polar-core/pull/727
...
* llama.cpp backend (CPU, Metal) now requires a redownload of gguf model due to upstream format changes: https://github.com/ha5a12/polar-core/pull/645 https://github.com/ggerganov/llama.cpp/pull/3252
* Due to indexing format changes, the `~/.polar/index` needs to be manually removed before any further runs of `polar scheduler`.
...
* Cpu backend is switched to llama.cpp: https://github.com/ha5a12/polar-core/pull/638
* add `server.completion_timeout` to control the code completion interface timeout: https://github.com/ha5a12/polar-core/pull/637
* Cuda backend is switched to llama.cpp: https://github.com/ha5a12/polar-core/pull/656
* Tokenizer implementation is switched to llama.cpp, so polar no longer need to download additional tokenizer file: https://github.com/ha5a12/polar-core/pull/683
* Fix deadlock issue reported in https://github.com/ha5a12/polar-core/issues/718

## v0.4.0 (10/24/2023)

### Features

* Supports golang: https://github.com/ha5a12/polar-core/issues/553
* Supports ruby: https://github.com/ha5a12/polar-core/pull/597
...
* Improve snippets retrieval by dedup candidates to existing content + snippets: https://github.com/ha5a12/polar-core/pull/582

## v0.3.1 (10/21/2023)
### Fixes and improvements

* Fix GPU OOM issue caused the parallelism: https://github.com/ha5a12/polar-core/issues/541, https://github.com/ha5a12/polar-core/issues/587
* Fix git safe directory check in docker: https://github.com/ha5a12/polar-core/issues/569
...
* Fix [Issue #511](https://github.com/ha5a12/polar-core/issues/511) by marking ggml models as optional.
...
```bash
polar serve --device metal --model PolarML/StarCoder-1B --chat-model PolarML/Mistral-7B
```

#### ModelScope Model Registry

Mainland Chinese users have been facing challenges accessing Hugging Face due to various reasons. The Polar team is actively working to address this issue by mirroring models to a hosting provider in mainland China called modelscope.cn.

```bash
## Download from the Modelscope registry
TABBY_REGISTRY=modelscope polar download --model PolarML/WizardCoder-1B
```

### Fixes and improvements

* Implemented more accurate UTF-8 incremental decoding in the [GitHub pull request](https://github.com/ha5a12/polar-core/pull/491).
* Fixed the stop words implementation by utilizing RegexSet to isolate the stop word group.
* Improved model downloading logic; now Polar will attempt to fetch the latest model version if there's a remote change, and the local cache key becomes stale.
